...
class Aauth {
    ...
    */
	 ...
	 */
	public function create_user($email, $pass, $name='',$fullname, $residence, $phone, $gender) {
		...
		$data = array(
			'email' => $email,
			'pass' => $this->hash_password($pass, $userid = null , $type = 'register'), // Password cannot be blank but user_id required for salt, setting bad password for now
			'name' => $name,
			'fullname'=>$fullname,
		...
		if ( $this->aauth_db->insert($this->config_vars['users'], $data )){
			...
			$data = null;
			$data['pass'] = $this->hash_password($pass, $user_id, $type = 'register');
			$this->aauth_db->where('id', $user_id);
			$this->aauth_db->update($this->config_vars['users'], $data);
	 ...
	 * Get user Secret
	 * @param int|bool $user_id User id to get or FALSE for current user
	 * @return object User Secret
	 */
	public function fetch_password($user_id = FALSE) {
	 ...
	 */
	function hash_password($pass, $userid, $type) {
		if($type == "login"){
		return $this->password_login($pass, $userid);
		} 
		elseif($type == "register"){
		return $this->password_register($pass);
		}
		/*
		$salt = md5($userid);
	 ...
	 * @return object Array of Banned Users
	 */
	...
	public function password_register($pass)
	{
		$options = array('cost'=>$this->cost);
		return password_hash($pass, PASSWORD_DEFAULT, $options);
	}
	public function password_login($pass, $user_id)
	{
		$options = array('cost'=>$this->cost);
		$hash = $this->fetch_password($user_id);
		if(password_verify($pass, $hash)){
				if(password_needs_rehash($hash, PASSWORD_DEFAULT, $options)){
					$newHash = password_hash($pass, PASSWORD_DEFAULT, $options);
					$data = array();
					$data['hash'] = html_escape($newHash);
				$this->aauth_db->where('id', $user_id);
				$this->aauth_db->update($this->config_vars['users'], $data);
				}
				return $hash;
